...
var ContextToastr = function ContextToastr(props) {
  ...
  var dispatcher = function dispatcher(action, payload) {
    return function () {
  ...
  (0, _react.useEffect)(function () {
    ...
    _toastrEmitter.EE.on('toastr/confirm', dispatcher(showConfirm));

    _toastrEmitter.EE.on('add/toastr', addDispatcher);

    _toastrEmitter.EE.on('clean/toastr', dispatcher(clean));

    _toastrEmitter.EE.on('removeByType/toastr', dispatcher(removeByType));

    _toastrEmitter.EE.on('remove/toastr', dispatcher(remove));
...
var testReducer = function testReducer() {
  ...
  var toastr = arguments.length > 1 ? arguments[1] : undefined;
